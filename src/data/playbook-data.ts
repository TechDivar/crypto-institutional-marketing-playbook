export interface Chapter {
  id: number;
  title: string;
  subtitle?: string;
}

export const chapters: Chapter[] = [
  { id: 1, title: "What the Data Says", subtitle: "Skills, salaries, and backgrounds from 23 real job descriptions." },
  { id: 2, title: "What Is Institutional Marketing?", subtitle: "It's not about going viral on X or building a community. This one is different." },
  { id: 3, title: "What Institutions Need From DeFi", subtitle: "Privacy, compliance, and risk — the 3 things that actually matters." },
  { id: 4, title: "Who Are You Selling To?", subtitle: "Your target audience." },
  { id: 5, title: "Where They Hang Out", subtitle: "Not Twitter. Not Discord." },
  { id: 6, title: "What Content to Make", subtitle: "Content that helps them say yes." },
  { id: 7, title: "Founder Content Is Your Cheapest Trust Signal", subtitle: "The compliance officer googles your CEO before the deal is approved." },
  { id: 8, title: "The Sales Cycle", subtitle: "Stage by stage, month by month." },
  { id: 9, title: "Common Mistakes", subtitle: "Consumer crypto logic vs. institutional reality." },
  { id: 10, title: "Messaging Rules", subtitle: "How to land in a boardroom and not get ghosted." },
  { id: 11, title: "The Launch Playbook", subtitle: "From positioning to post-launch." },
  { id: 12, title: "Metrics and KPIs", subtitle: "What actually matters and what does not." },
  { id: 13, title: "How to Get One of These Jobs", subtitle: "What the JDs actually want." },
  { id: 14, title: "Test Yourself", subtitle: "What you should know before any interview." },
  { id: 15, title: "Glossary", subtitle: "Your crypto institutional marketing dictionary." },
];
